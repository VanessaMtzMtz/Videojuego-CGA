...
float distanceFromTarget = 7.0; //distancia del personaje-camara
bool cambiaCam = true;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//No es necesario enviar la posición ya que la posición es parte del calculo de la camara en tercera persona
...
bool processInput(bool continueApplication) {
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS && glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
	{
		if (cambiaCam) 
		{
			cambiaCam = false; //Primera Persona
		}
		else 
		{
			cambiaCam = true;//Tercera Persona
		}
	}
...
void applicationLoop() {
	...
	while (psi) {
		...
		if (cambiaCam) { 
			view = camera->getViewMatrix();// Tercera Persona
		}
		else { 

			view = camera2->getViewMatrix();// Primera persona
		}
...
int main(int argc, char **argv) {
	init(800, 700, "Práctica 6 Martínez Martínez Vanessa", false);
	applicationLoop();
	destroy();
	return 1;
}
