...
void introduccionTextos() {
	int paginaIntroduccion = 0;
	if (glfwGetKey(window, GLFW_KEY_ENTER) == GLFW_PRESS)
	{
		paginaIntroduccion = paginaIntroduccion + 1;
	}
	switch (paginaIntroduccion) {
	case 0:
		modelText->render("Despues de las mortiferas hordas", -0.85, -0.35, 35, 1.0, 1.0, 1.0);
		modelText->render("de Covid, cuando todo parecia perdido", -0.85, -0.5, 35, 1.0, 1.0, 1.0);
		modelText->render("Presione ENTER para continuar", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;

	case 1:
		modelText->render("Un heroe asomaba su presencia, anunciando", -0.85, -0.35, 35, 1.0, 1.0, 1.0);
		modelText->render("la ayuda que tanto necesitamos.", -0.85, -0.5, 35, 1.0, 1.0, 1.0);
		modelText->render("Presione ENTER para continuar", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;

	case 2:
		modelText->render("Con su gran bata blanca y sus medicinas", -0.85, -0.35, 35, 1.0, 1.0, 1.0);
		modelText->render("Llega nuestro doctor simi para salvarnos", -0.85, -0.5, 35, 1.0, 1.0, 1.0);
		modelText->render("Presione ENTER para continuar", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;

	case 3:
		modelText->render("El se enfrentara con todos los ", -0.85, -0.35, 35, 1.0, 1.0, 1.0);
		modelText->render("malevolos virus, poniendo", -0.85, -0.5, 35, 1.0, 1.0, 1.0);
		modelText->render("su vida en riesgo con tal de salvarnos", -0.85, -0.65, 35, 1.0, 1.0, 1.0);
		modelText->render("Presione ENTER para continuar", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;

	case 4:
		modelText->render("Objetivo: Cruzar todo los virus hasta llegar al objetivo", -0.9, -0.65, 40, 0.3, 0.9, 0.7);
		modelText->render("Presione ENTER para continuar", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;

	case 5:
		modelText->render("Controles (PC): Flechas de movimiento y espacio", -0.35, -0.65, 40, 0.3, 0.9, 0.7);
		modelText->render("Presione ENTER para continuar", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;

	case 6:
		modelText->render("Controles (Mando):Jostick y boton A", -0.4, -0.65, 40, 0.3, 0.9, 0.7);
		modelText->render("Presione ENTER para comenzar a jugar y diviertete", -0.45, -0.85, 30, 1.0, 1.0, 1.0);
		break;
	case 7:
		applicationLoop();
	default:
