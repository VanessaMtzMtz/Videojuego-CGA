...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//Cyborg animación prueba
	cyborgAnimate.loadModel("../models/cyborg/PruebaAnimacion1.fbx");
	cyborgAnimate.setShader(&shaderMulLighting);

	//Gatito
	gatoAnimate.loadModel("../models/Lich/lich.fbx");
	gatoAnimate.setShader(&shaderMulLighting);
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 0 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS){
		modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0.0f, 0.0f, 0.01f));
	...
	if (modelSelected == 0 && glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
		modelMatrixMayow = glm::rotate(modelMatrixMayow, -0.01f, glm::vec3(0.0f, 1.0f, 0.0f));
		mayowModelAnimate.setAnimationIndex(0);
	...
	if (modelSelected == 5 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
		modelMatrixGato = glm::translate(modelMatrixGato, glm::vec3(0.01f, 0.0f, 0.0f));
		gatoAnimate.setAnimationIndex(0);
	}
	...
	if (modelSelected == 5 && glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
		modelMatrixGato = glm::rotate(modelMatrixGato, -0.01f, glm::vec3(0.0f, 1.0f, 0.0f));
		gatoAnimate.setAnimationIndex(0);
	}
	else if (modelSelected == 5 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
		modelMatrixGato = glm::rotate(modelMatrixGato, 0.01f, glm::vec3(0.0f, 1.0f, 0.0f));
		gatoAnimate.setAnimationIndex(0);
	}
...
void applicationLoop() {
	...
	modelMatrixCyborg = glm::translate(modelMatrixCyborg, glm::vec3(-3.0f, 0.05f, -10.0f));

	modelMatrixGato = glm::translate(modelMatrixGato, glm::vec3(0.0f, 0.0f, 0.0f));
	...
	while (psi) {
		...
		cyborgAnimate.setAnimationIndex(0);//cambiar animacionea
		cyborgAnimate.render(modelMatrixCyborgBody);

		glm::mat4 modelMatrixGatoBody = glm::mat4(modelMatrixGato);
		modelMatrixGatoBody = glm::scale(modelMatrixGatoBody, glm::vec3(0.01f, 0.01f, 0.01f));
		gatoAnimate.render(modelMatrixGatoBody);
		gatoAnimate.setAnimationIndex(1);
